    ...
    <main id="main">
        ...
        <div class="page-headers">
            <div class="nave-view">
                <ol class="breadcrumb animated slideInRight  mb-0">
                    ...
                    <!-- <li class="breadcrumb-item"><a href="index.php">Home</a></li> -->
                    <?php
                    $VideoCategory = $_GET['CID'];
                    $FolderId = $_GET['FID'];
                    if($VideoCategory == 1){
                        $CategoryLink = 'VideoMaterials.php';
                    }elseif($VideoCategory == 3){
                        $CategoryLink = 'VideoProjects.php';
                    }
                    else{
                        $CategoryLink = 'VideoGalleryOthers.php';
                    }
                    
                    if ($_GET['FID'] != '' && $_GET['SFID'] == '') {

                        $FindCategory = mysqli_query($con, "SELECT categoryName FROM category_table WHERE id = '$VideoCategory'");
                        foreach($FindCategory as $FindCategoryResult){
                            $Category = $FindCategoryResult['categoryName'];
                        }
                        $FindFolder = mysqli_query($con, "SELECT folderName FROM folder_table WHERE id = '$FolderId'");
                        foreach($FindFolder as $FindFolderResult){
                            $Folder = $FindFolderResult['folderName'];
                        }
                        echo '<li class="breadcrumb-item active" aria-current="page"><a href="'.$CategoryLink.'">'.$Category.'</a></li>';
                        echo '<li class="breadcrumb-item active" aria-current="page">'.$Folder.'</li>';
                    }elseif($_GET['FID'] != '' && $_GET['SFID'] != ''){
                        $SubFolderId = $_GET['SFID'];
                        $FindCategory = mysqli_query($con, "SELECT categoryName FROM category_table WHERE id = '$VideoCategory'");
                        foreach($FindCategory as $FindCategoryResult){
                            $Category = $FindCategoryResult['categoryName'];
                        }
                        ...
                        $FindSubFolder = mysqli_query($con, "SELECT subFolderName FROM subfolder_table WHERE id = '$SubFolderId'");
                        foreach($FindSubFolder as $FindSubFolderResult){
                            $SubFolder = $FindSubFolderResult['subFolderName'];
                        }
                        echo '<li class="breadcrumb-item active" aria-current="page"> <a href="'.$CategoryLink.'">'.$Category.'</a></li>';
                        echo '<li class="breadcrumb-item active" aria-current="page"><a href="VideoSubFolders.php?CID='.$VideoCategory.'&FID=' .$FolderId. '&SFID=">'.$Folder.'</a></li>';
                        echo '<li class="breadcrumb-item active" aria-current="page">'.$SubFolder.'</li>';
                    }
                    ?>
        ...
        <section id="gallery" class="gallery section-bg mt-5">
            <div class="container" data-aos="fade-up">
                ...
                if ($_GET['FID'] != '' && $_GET['SFID'] == '') {
                    //$SubFolderId = $_GET['FID'];
                    ?>

                    <div class="row">
                        <?php

                        $ListFolders = mysqli_query($con, "SELECT id,subFolderName FROM subfolder_table WHERE folderId = '$FolderId'");
                        if (mysqli_num_rows($ListFolders) > 0) {
                            foreach ($ListFolders as $ListFoldersResults) {
                                echo '
                                <div class="col-xl-3 col-lg-4 col-md-6 col-6 mb-3">
                                    <a href="VideoSubFolders.php?CID='.$VideoCategory.'&FID=' .$FolderId. '&SFID='.$ListFoldersResults["id"].'">
                                        <div class="card-body Folder">
                                            <img src="./img/folder.jpg" class="img-fluid">
                                            <h6 class="text-center">' . $ListFoldersResults["subFolderName"] . '</h6>
                                        </div>
                                    </a>
                                </div>';
                            }
                    ...
                    <div class="row VideoClientSideRow">
                        <?php
                        $FetchVideos = mysqli_query($con, "SELECT * FROM video_table WHERE videoCategory = '$VideoCategory' AND videoFolder = '$FolderId' AND videoSubFolder = 0");
                        foreach ($FetchVideos as $FetchVideoResults) {
                            echo '<div class="col-xl-3 col-lg-3 col-12 mb-4">
                                <a class="VideoCientSide" href="VideoPlay.php?ID=' . $FetchVideoResults["videoId"] . '&VCat=' . $VideoCategory . '&VFol=' . $FolderId . '&VSFol=">
                ...
                if ($_GET['SFID'] != '') {
                    ...
                    <div class="row VideoClientSideRow">
                        <?php
                        $FetchVideos = mysqli_query($con, "SELECT * FROM video_table WHERE videoCategory = '$VideoCategory' AND videoFolder = '$FolderId' AND videoSubFolder = '$SubFolderId'");
                        foreach ($FetchVideos as $FetchVideoResults) {
                            echo '<div class="col-xl-3 col-lg-3 col-12 mb-4">
                                <a class="VideoCientSide" href="VideoPlay.php?ID=' . $FetchVideoResults["videoId"] . '&VCat=' . $VideoCategory . '&VFol=' . $FolderId . '&VSFol=' . $SubFolderId . '">
                                    <img class="img-fluid ThumbnailImage" src="./THUMBNAILS/' . $FetchVideoResults["videoThumbnail"] . '" alt="event">
                                    <div class="videoOverlay"> <img src="img/play.png"> </div>
                                </a>
                                <h6 class="mt-3 ps-3"> ' . $FetchVideoResults['videoHeader'] . ' </h6>
